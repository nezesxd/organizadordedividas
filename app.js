...
import { getFirestore, collection, addDoc, getDocs, query, where, serverTimestamp, doc, updateDoc, deleteDoc } from "https://www.gstatic.com/firebasejs/10.4.0/firebase-firestore.js";
...
const btnSair = document.getElementById('btnSair'); // O novo botão na Navbar
...
// 0. Monitor de Autenticação (Protege a página)
onAuthStateChanged(auth, (user) => {
    if (user) {
        usuarioAtual = user;
        carregarMeses(); // Só carrega os dados DEPOIS de confirmar que está logado
    } else {
        window.location.href = "login.html"; // Expulsa para o login se não estiver autenticado
    }
});
...
// 1. Carregar Meses (Filtrado pelo Usuário Logado)
async function carregarMeses() {
    ...
    // Puxa APENAS os meses criados pelo usuário logado
    const q = query(collection(db, "meses"), where("userId", "==", usuarioAtual.uid));
    const querySnapshot = await getDocs(q);
    
    // Organiza os meses do mais novo para o mais velho (via JavaScript para evitar erros de índice no Firebase)
    let arrayMeses = [];
    querySnapshot.forEach((doc) => {
        arrayMeses.push({ id: doc.id, ...doc.data() });
    });
    arrayMeses.sort((a, b) => (b.createdAt?.toMillis() || 0) - (a.createdAt?.toMillis() || 0));

    selectMeses.innerHTML = '<option value="" disabled selected>Escolha um mês...</option>';
    
    arrayMeses.forEach((dados) => {
        selectMeses.insertAdjacentHTML('beforeend', `<option value="${dados.id}">${dados.nome}</option>`);
        mesDividaModal.insertAdjacentHTML('beforeend', `<option value="${dados.id}">${dados.nome}</option>`);
        mesDividaModalEdit.insertAdjacentHTML('beforeend', `<option value="${dados.id}">${dados.nome}</option>`);
        
        listaMesesParcelamento.insertAdjacentHTML('beforeend', `
            <div class="form-check">
                <input class="form-check-input chk-mes-extra" type="checkbox" value="${dados.id}" id="chk_${dados.id}">
                <label class="form-check-label text-secondary" for="chk_${dados.id}">
                    ${dados.nome}
                </label>
            </div>
        `);
...
// 6. Formulários Submit (Cadastrar Meses e Dívidas vinculando ao Usuário)
...
formMes.addEventListener('submit', async (e) => {
    ...
    try {
        await addDoc(collection(db, "meses"), { 
            nome: document.getElementById('nomeMes').value, 
            userId: usuarioAtual.uid, // <-- Vínculo do Mês com o Usuário
...
formDivida.addEventListener('submit', async (e) => {
    ...
    // Adicionamos o userId no payload da dívida também
    const dadosGerais = {
        userId: usuarioAtual.uid, // <-- Vínculo da Dívida com o Usuário
...
// A remoção da chamada inicial (DOMContentLoaded) foi feita de propósito.
// Agora, quem inicia o carregamento dos meses é o onAuthStateChanged, lá na linha 28.